...
const AGENT_URL = 'https://efcx-qa9.expertflow.com/unified-agent/#/login';
...
const CUSTOMER_WIDGET_URL = 'https://efcx-qa9.expertflow.com/customer-widget/#/widget?widgetIdentifier=Web&serviceIdentifier=1122'
//const AGENT_CREDENTIALS = { username: 'hassan', password: '12345' };
...
async function startCustomerChat(page: any, customerInfo: { name: string; phoneNumber: string; phoneNumber1: string }) {
...
//  await page.locator('div').filter({ hasText: /^phone\*$/ }).getByRole('textbox').fill(customerInfo.phone);
  await page.locator('input#phone').fill(customerInfo.phoneNumber1); //customerInfo.phone
...
let isHumanAgentSelected = false; // Flag to ensure "Talk to a human agent" is clicked only once
...
test('Record Chat', async ({ page, browser }) => {
  ...
  await loginPage.login(agentInfo.agentID, agentInfo.password);
  //await loginAgent(page, agentInfo);
  await setAgentStatusToReady(page);

  // Customer starts chat
  await startCustomerChat(page1, customerInfo);
  await sendMessage(page1, 'hi');

  // Ensure "Talk to a human agent" is clicked only once
  if (!isHumanAgentSelected) {
    await page1.getByRole('option', { name: 'Talk to a human agent' }).click();
    isHumanAgentSelected = true; // Update flag after execution
  }
